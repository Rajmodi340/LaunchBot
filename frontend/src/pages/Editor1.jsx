...
function Editor1() {
  ...
  useEffect(() => {
    const handlegetwebsite = async () => {
      try {
        ...
        const sorted = [...result.data.conversation].sort(
  ...
  if (error) {
    return (
      <div className="h-screen flex-center justify-center bg-black text-red-400">
        {error}
      </div>
    );
  }
          ...
          <div className="p-3 border-t border-white/10">
            <div className="flex flex-col gap-2 mt-100">
              ...
              <button
                ...
                disabled={updateLoading || !prombt.trim()}
              >
                <Send size={14}></Send>
              </button>
              {inputError && (
                <p className="text-red-400 text-xs italic mt-1">{inputError}</p>
              )}
              {updateError && (
      ...
      <div className="flex flex-1 flex-col">
        <div className="h-14 px-4 flex justify-between items-center border-b border-white/10 bg-black/80">
          ...
          <div className="flex items-center gap-2">
            <button
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gradient-to-r from-indigo-500 to-purple-500 text-sm font-semibold hover:scale-105 transition-transform disabled:opacity-50"
              onClick={handleDeploy}
              disabled={deployLoading || website?.deployed}
            >
              <Rocket size={14} />
        ...
        {deployMessage && (
          <div
            className={`px-4 py-2 text-sm ${deployMessage.startsWith("Error") ? "bg-red-900/30 text-red-300" : "bg-green-900/30 text-green-300"}`}
          >
            {deployMessage}
          </div>
        )}
